...
import { ArrowLeft, CalendarDays, CheckCircle2, ChevronDown, Clock3, Info, Sprout } from "lucide-react";
...
function PhenologyDetail({ data }: { data: FenologiaEstimada }) {
  return <>
    ...
    <Facts data={data} />
    <section className="mt-8 flex gap-3 rounded-xl border border-[#9b7848] bg-[#f3ece3] p-4 sm:p-5"><Info className="mt-0.5 h-5 w-5 shrink-0 text-[#9b7848]" /><p className="text-base leading-6 text-[#405369]">Esta estimación ayuda a interpretar las condiciones actuales según el momento probable del cultivo. Todavía no modifica las señales ni reemplaza la observación a campo o la recomendación de un asesor agronómico.</p></section>
  </>;
}
...
function MobileTimeline({ data }: { data: FenologiaEstimada }) {
  ...
  return <div className="relative mt-3 space-y-2 lg:hidden"><div className="absolute bottom-6 left-[106px] top-6 w-1 bg-[#e1e5e8]" aria-hidden="true" />{data.hitos.map(hito => {
    const active = hito.codigo === data.estadio_actual_estimado, critical = CRITICAL_STAGES.has(hito.codigo), open = expanded === hito.codigo;
    return <article key={hito.codigo} className={`relative overflow-hidden rounded-xl border-2 bg-white ${active ? "border-[#70bd91] bg-[#edfaf2]" : "border-[#e1e5e8]"}`}>
      <button type="button" onClick={() => setExpanded(open ? null : hito.codigo)} aria-expanded={open} className="relative z-10 grid min-h-[86px] w-full grid-cols-[66px_62px_1fr_24px] items-center gap-2 px-3 text-left">
        <img src={STAGE_IMAGES[hito.codigo]} alt="" className="mx-auto h-[70px] w-[64px] object-contain" />
        ...
        <span className="min-w-0">{active && <span className="mb-1 inline-flex rounded-full border border-[#2e8537] bg-white px-2 py-0.5 text-xs text-[#2e8537]">Estado actual estimado</span>}<strong className="block text-base leading-[1.05]">{hito.nombre}</strong><span className="mt-1 flex items-center justify-between text-base font-bold"><span>{shortDate(hito.fecha_estimada)}</span>{active && <span className="text-sm text-[#2e8537]">± {data.margen_dias} días</span>}</span></span>
      ...
      {open && <div className="relative z-10 grid min-h-[300px] grid-cols-[106px_1fr] border-t border-[#eef0f2] bg-white"><div className="flex items-center justify-center border-r-[10px] border-[#e1e5e8]">{critical && <span className="flex -rotate-90 items-center gap-2 whitespace-nowrap text-lg font-bold text-[#eb5505]"><CriticalIcon /> Período crítico</span>}</div><div className="flex flex-col items-center justify-center p-5 text-center"><img src={STAGE_IMAGES[hito.codigo]} alt={`Ilustración de ${hito.nombre}`} className="h-44 w-full object-contain" /><span className="mt-2 text-xl text-[#526477]">Fecha estimada</span><strong className="mt-1 text-xl">{shortDate(hito.fecha_estimada)}</strong></div></div>}
    </article>;
  })}</div>;
}
...
function DesktopStage({ hito, active, margin }: { hito: HitoFenologico; active: boolean; margin: number }) {
  return <div className={`relative z-10 flex h-[410px] flex-col items-center justify-end px-3 text-center ${active ? "rounded-2xl border-2 border-[#70bd91] bg-[#edfaf2]" : ""}`}>
    ...
    <img src={STAGE_IMAGES[hito.codigo]} alt={`Ilustración de ${hito.nombre}`} className="mb-1 h-[170px] w-full object-contain" />{active && <strong className="mb-3 text-sm text-[#176d35]">{hito.nombre}</strong>}<span className={`h-5 w-5 rounded-full border-[3px] ${active ? "border-[#b9dcc8] bg-[#087b4b]" : "border-[#e1e5e8] bg-white"}`} />
