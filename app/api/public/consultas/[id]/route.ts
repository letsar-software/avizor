...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  const rid=requestId(request); try { if(!hasDatabaseConfig()) throw new DomainError("CONSULTA_NO_ENCONTRADA","No encontramos esa consulta.",404);
    const result=await query<{resultado:unknown}>("select coalesce(resultados_json,resultado) resultado from consultas where share_token=$1 or id::text=$1",[params.id]);
