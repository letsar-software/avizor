...
**Hallazgo colateral, resuelto aparte**: verificando el flujo público real contra Railway durante este mismo trabajo, `lib/pests/zone-resolver.ts` fallaba con `column z.estado does not exist` — no se tocó en esta rama porque es un bug de otro módulo, no del motor de evaluación. Se resolvió en paralelo ([PR #17](https://github.com/letsar-software/avizor/pull/17), ya mergeado); ver el detalle en la sección siguiente.
