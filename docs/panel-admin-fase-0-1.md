...
## Motor de plagas ahora reutiliza el motor genérico de indicadores (resuelto, 2026-08-26)

Al elegir `lib/pests/` como motor definitivo quedó una deuda pendiente (punto 9 del documento de pendientes): su cálculo climático (`PestRulesEngine`, función `calculate()` interna) reimplementaba a mano la agregación de series climáticas — indicadores hardcodeados con `if (key.startsWith("precip_"))`, en vez del `IndicatorEngine` genérico que ya usan las reglas de enfermedades (`lib/rules/engine-v2.ts`). Sumar un indicador nuevo (ej. resolver PEND-19, la variable térmica de arañuela) requería editar código, no cargar dato — contradice la premisa central del proyecto.

**Resuelto en la rama `refactor/motor-plagas-data-driven`:**
- `lib/indicators/engine.ts`: se extrajo `AgregacionInput` (`Pick<CondicionDefinicion, "variable"|"agregador"|"cobertura_minima"|"subcondicion">`) como lo mínimo que `IndicatorEngine.calculate()` necesita — `CondicionDefinicion` sigue satisfaciéndolo estructuralmente, así que `engine-v2.ts` no cambió una línea.
- `lib/pests/indicator-spec.ts` (nuevo): traduce cada `IndicadorPlaga` con nombre (`temp_media_7d`, `precip_10d`, etc.) a `{variable, agregador, ventanaDias, subcondicion?}` del motor genérico. `cobertura_minima` siempre `1` explícito, porque el motor de plagas históricamente exigía la ventana completa sin días faltantes para cualquier indicador — más estricto que el 80% que `engine-v2` tolera por defecto en `media_ventana` — así que se preservó ese criterio en vez de heredar el default.
- `lib/pests/engine.ts`: `PestRulesEngine` recibe un `IndicatorEngine` inyectado (mismo patrón de DI que el resto del proyecto) y delega todo el cómputo a `resolverIndicadorPlaga()` + `this.indicators.calculate()`. Se eliminaron `complete`/`validCount`/`sum`/`metric`/`compare`/el `calculate()` ad-hoc (reemplazado por `evaluateOperator` del motor genérico, ya sin duplicar la comparación de operadores).
- **No se tocó** el formato de `reglas_plagas.configuracion` (`condiciones: [{indicador, operador, valor}]`) ni la migración `013` — cero cambio de dato, solo de implementación. Los 21 tests existentes de `tests/plagas.test.ts`/`tests/pest-results.test.ts` (que ejercitan P-01 con el formato real de producción) pasan sin modificar ninguna aserción, confirmando cero cambio de comportamiento.
- **Queda documentado, no resuelto**: `dias_consecutivos_sin_lluvia` (racha máxima sin lluvia) sigue sin agregador en `IndicatorEngine` — `resolverIndicadorPlaga` devuelve `null` explícitamente para ese caso. No se agregó un agregador nuevo porque nadie lo evalúa hoy (P-02/arañuela sigue con `niveles: []`, inactiva, pendiente de PEND-19). Cuando se resuelva PEND-19, agregar el agregador (`racha_maxima_consecutiva` o similar) a `IndicatorEngine` es el único paso que falta — el resto del cableado ya está.

**Hallazgo colateral (no arreglado en esta rama, tarea aparte)**: verificando el flujo público real contra Railway, `lib/pests/zone-resolver.ts` falla con `column z.estado does not exist` — `zonas_agronomicas` (migración `012`) tiene `activa boolean`, no `estado`. El bug nunca se manifestó porque `ENABLE_PLAGAS` no está seteado en producción (confirmado vía Railway `get-service-config`), así que toda la sección de plagas del flujo público está apagada hoy. Si alguien prende el flag tal cual está el código, la consulta pública va a devolver 500 en cuanto intente resolver zona. Queda anotado como tarea separada, no se tocó acá porque es un bug de otro módulo (`zone-resolver.ts`), no del motor de evaluación.