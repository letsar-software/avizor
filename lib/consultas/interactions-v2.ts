...
async function resolve(id: string) {
  ...
  const result=await query<{id:string;cultivo:string;localidad_normalizada:string}>("select id::text,cultivo,localidad_normalizada from consultas where share_token=$1 and created_at >= now() - interval '30 days'",[id]);
