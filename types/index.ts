...
export type EstadoCoberturaAnalisis = "datos_completos" | "datos_parciales" | "datos_insuficientes";
export interface EvaluacionLimitadaPorClima { riesgo: string; categoria?: string; variable?: string; dias_esperados: number; dias_disponibles: number; dias_faltantes: number; cobertura: number; motivo: "datos_insuficientes"; }
export interface CoberturaAnalisis { cobertura: number; estado: EstadoCoberturaAnalisis; variables_faltantes_relevantes: string[]; evaluaciones_limitadas: EvaluacionLimitadaPorClima[]; }
...
export interface ResultadoConsultaV2Publica { id: string | null; request_id: string; share_token: string; estado_general: EstadoGeneral; explicacion: string; resumen_consulta?: { descripcion: string; destaque: string }; localidad: LocalidadNormalizada | null; cultivo: string; fecha_ref: string; generado_en: string; proveedor_climatico: string; reglas: ResultadoReglaV2[]; plagas?: { evaluaciones: EvaluacionPlaga[]; disponibilidad: "disponible" | "zona_no_resuelta" }; contexto_fenologico: ContextoFenologico; duracion_ms: number; clima: { serie: SerieClimaticaDiaria[]; rango_temporal: { desde: string; hasta: string }; cobertura: number; variables_disponibles: string[]; variables_faltantes?: string[]; dias_solicitados?: number; dias_disponibles?: number; obtenido_en?: string; adapter_version: string }; calidad_analisis?: CoberturaAnalisis; prevision?: PrevisionClimaticaDiaria[]; }
